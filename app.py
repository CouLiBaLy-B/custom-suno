"""
AI Music Studio — Single-file Application for Hugging Face Spaces
Architecture plate : zéro problème d'import, zéro package.
"""
...
class MusicGenService:
    ...
    def load_model(self):
        ...
        try:
            ...
            print("✅ MusicGen loaded (audiocraft)")
        except ImportError:
            ...
            model = MusicgenForConditionalGeneration.from_pretrained(settings.musicgen_model, torch_dtype=dtype)
            model = model.to(self.device)
            self._model = model
            self._processor = proc
            print("✅ MusicGen loaded (transformers)")
    async def generate(self, prompt, duration=15, temperature=1.0, top_k=250, top_p=0.0, cfg_coef=3.0, num_variations=1, seed=-1, progress_callback=None):
        self.load_model()
        ...
        if hasattr(self._model, 'set_generation_params'):
            self._model.set_generation_params(duration=min(duration,30), top_k=top_k, top_p=top_p, temperature=temperature, cfg_coef=cfg_coef)
            ...
            audio_values = self._model.generate(**inputs, max_new_tokens=int(duration*50), do_sample=True, temperature=temperature, top_k=top_k, top_p=top_p)
            audio = audio_values.cpu().numpy()
            sr = self._model.config.audio_encoder_sample_rate
...
class StableAudioService:
    ...
    def load_pipeline(self):
        ...
        self._pipeline = StableAudioPipeline.from_pretrained(settings.stable_audio_model, torch_dtype=dtype)
        self._pipeline = self._pipeline.to(self.device)
        print("✅ Stable Audio loaded")
    async def generate(self, prompt, negative_prompt=None, duration=30.0, num_variations=1, seed=-1, progress_callback=None):
        self.load_pipeline()
        if progress_callback: progress_callback(20)
        import torch
        generator = torch.Generator(device=self.device).manual_seed(seed) if seed >= 0 else None
        if progress_callback: progress_callback(40)
        result = self._pipeline(prompt=prompt, negative_prompt=negative_prompt or "", num_inference_steps=200, audio_end_in_s=min(duration,47.0), num_waveforms_per_prompt=num_variations, generator=generator)
        if progress_callback: progress_callback(90)
        audio = result.audios[0].cpu().numpy() if num_variations == 1 else result.audios.cpu().numpy()
        sr = self._pipeline.vae.sampling_rate
        if progress_callback: progress_callback(100)
        return audio, sr
...
class BarkService:
    ...
    def load_model(self):
        ...
        self._model = self._model.to(self.device)
        print("✅ Bark loaded")
    async def generate(self, text, voice_preset="v2/fr_speaker_1", duration=None, num_variations=1, seed=-1, progress_callback=None):
        self.load_model()
        if progress_callback: progress_callback(20)
        ...
        for i in range(num_variations):
            if seed >= 0: torch.manual_seed(seed + i)
            audio = self._model.generate(**{k: v.to(self.device) for k, v in inputs.items()}, do_sample=True)
...
app = FastAPI(title="AI Music Studio", version="0.1.0", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

@app.post("/api/generate", response_model=GenerationResponse)
async def generate_music(request: GenerationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    gen = Generation(
        ...
        parameters=json.dumps({"duration": request.duration, "temperature": request.temperature,
    ...
    db.add(gen)
    db.commit()
    _task_store[task_id] = {"task_id": task_id, "status": "pending", "progress": 0, "created_at": datetime.utcnow().isoformat()}
    background_tasks.add_task(_run_generation, task_id=task_id, request=request)
    return GenerationResponse(task_id=task_id, estimated_time_seconds=request.duration * 2,
...
async def get_status(task_id: str):
    ...
    resp = GenerationStatus(task_id=task_id, status=task["status"], progress=task.get("progress", 0), current_step=task.get("current_step"))
    if task["status"] == "completed":
        resp.result = {"task_id": task_id, "audio_url": task.get("audio_url"), "audio_duration": task.get("audio_duration")}
    ...
    try:
        while True:
            if task_id not in _task_store:
                ...
                break
            task = _task_store[task_id]
            await websocket.send_json({"task_id": task_id, "status": task["status"], "progress": task.get("progress", 0), "current_step": task.get("current_step")})
...
async def list_projects(db: Session = Depends(get_db)):
    ...
    return [ProjectResponse(id=p.id, name=p.name, description=p.description, created_at=p.created_at.isoformat()) for p in projects]

@app.post("/api/projects", response_model=ProjectResponse)
async def create_project(proj: ProjectCreate, db: Session = Depends(get_db)):
    new_proj = Project(id=str(uuid.uuid4()), owner_id="anonymous", name=proj.name, description=proj.description, created_at=datetime.utcnow())
    db.add(new_proj)
    db.commit()
    db.refresh(new_proj)
    return ProjectResponse(id=new_proj.id, name=new_proj.name, description=new_proj.description, created_at=new_proj.created_at.isoformat())
...
async def login(creds: LoginRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        user = User(id=str(uuid.uuid4()), username=creds.username, email=f"{creds.username}@example.com", hashed_password=creds.password, created_at=datetime.utcnow())
...
async def _run_generation(task_id: str, request: GenerationRequest):
    ...
    try:
        if request.model_name == "musicgen":
            audio, sr = await MusicGenService().generate(prompt=request.prompt, duration=request.duration, temperature=request.temperature, top_k=request.top_k, top_p=request.top_p, cfg_coef=request.cfg_coef, num_variations=request.num_variations, seed=request.seed, progress_callback=lambda p: _update_task(task_id, progress=p))
        elif request.model_name == "stable_audio":
            audio, sr = await StableAudioService().generate(prompt=request.prompt, negative_prompt=request.negative_prompt, duration=request.duration, num_variations=request.num_variations, seed=request.seed, progress_callback=lambda p: _update_task(task_id, progress=p))
        elif request.model_name == "bark":
            audio, sr = await BarkService().generate(text=request.prompt, duration=request.duration, num_variations=request.num_variations, seed=request.seed, progress_callback=lambda p: _update_task(task_id, progress=p))
        else:
        ...
        with get_db_context() as db:
            ...
            if gen:
                gen.status = "completed"; gen.audio_url = f"/api/audio/{task_id}.wav"; gen.audio_duration = duration_sec; gen.completed_at = datetime.utcnow(); db.commit()
        _update_task(task_id, status="completed", progress=100, audio_url=f"/api/audio/{task_id}.wav", audio_duration=duration_sec, completed_at=datetime.utcnow().isoformat())
    except Exception as e:
        print(f"Échec génération {task_id}: {e}")
        _update_task(task_id, status="failed", error=str(e))
        with get_db_context() as db:
            gen = db.query(Generation).filter(Generation.id == task_id).first()
            if gen: gen.status = "failed"; gen.error_message = str(e); gen.completed_at = datetime.utcnow(); db.commit()

# ── hf_app.py entry point (quand lancé via python hf_app.py) ──
def launch_api():
    import subprocess
    print("🚀 Starting FastAPI backend on port 8000...")
    env = os.environ.copy()
    env["PYTHONPATH"] = app_dir + ":" + env.get("PYTHONPATH", "")
    subprocess.run([sys.executable, "-m", "uvicorn", "app:app", "--host", "0.0.0.0", "--port", "8000", "--log-level", "info"], env=env, cwd=app_dir)
...
def launch_frontend():
    ...
    if os.path.exists(frontend_file):
        subprocess.run([sys.executable, "-m", "streamlit", "run", frontend_file, "--server.port=7860", "--server.address=0.0.0.0", "--server.headless=true"], env=env, cwd=app_dir)
    else:
